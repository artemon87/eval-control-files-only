...
function TrendChart({ points, type, showPercentLabels }: { points: DailyTrend[]; type: "all" | EvalType; showPercentLabels: boolean }) {
  const width = 660;
  const top = 18;
  const bottom = 180;
  ...
  return (
    <div className="trend-chart" role="img" aria-label="Pass rate trend for E2E and unit evaluations">
      ...
      <svg viewBox="0 0 660 210" preserveAspectRatio="none" aria-hidden="true">
        ...
        {showPercentLabels && visibleTypes.flatMap((evalType) => points.map((point, index) => typeof point[evalType] === "number" ? <text key={`label-${evalType}-${point.date}`} className={`trend-value trend-value--${evalType}`} x={x(index)} y={Math.max(11, y(point[evalType]!) + (evalType === "e2e" ? -8 : 13))} textAnchor="middle">{point[evalType]!.toFixed(point[evalType]! % 1 ? 1 : 0)}%</text> : null))}
      </svg>
        ...
        <div className="content">
          {view === "overview" && <>
            ...
            <section className="dashboard-grid">
              <article className="panel trend-panel"><div className="panel-heading"><div><h2>Pass rate trend</h2><p>Case-weighted daily result from completed live runs</p></div><div className="chart-heading-actions"><label className="chart-label-toggle"><input type="checkbox" checked={overviewShowPercentLabels} onChange={(event) => setOverviewShowPercentLabels(event.target.checked)} /><span>Show % labels</span></label><div className="legend">{overviewType !== "unit" && <span><i className="e2e-dot" />E2E</span>}{overviewType !== "e2e" && <span><i className="unit-dot" />Unit</span>}</div></div></div><TrendChart points={overview.daily} type={overviewType} showPercentLabels={overviewShowPercentLabels} /></article>
...
function HistoryView({ initialFocus, runs, openingRunId, onOpenRun }: { initialFocus: HistoryFocus; runs: EvalRun[]; openingRunId: string | null; onOpenRun: (runId: string, evalType: EvalType) => void }) {
  ...
  return <>
    ...
    {trend.length ? <HistoryChart points={trend} title={historyType === "e2e" ? (target === "all" ? `${stage} · stage rollup` : target) : selectedSkill.label} windowDays={rangeDays} windowEnd={referenceTime} showPercentLabels={showPercentLabels} onShowPercentLabelsChange={setShowPercentLabels}/> : <section className="panel empty"><strong>No historical runs in the selected {rangeDays}-day window</strong><span>Change the time window, environment or scope selection.</span></section>}
