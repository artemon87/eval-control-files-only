...
function HistoryView({ initialFocus, runs, openingRunId, onOpenRun }: { initialFocus: HistoryFocus; runs: EvalRun[]; openingRunId: string | null; onOpenRun: (runId: string, evalType: EvalType) => void }) {
  ...
  return <>
    ...
    {trend.length ? <HistoryChart points={trend} title={historyType === "e2e" ? (target === "all" ? `${stage} · stage rollup` : target) : selectedSkill.label}/> : <section className="panel empty"><strong>No historical runs in the selected {rangeDays}-day window</strong><span>Change the time window, environment or scope selection.</span></section>}
